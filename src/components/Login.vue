<template>
    ...
    <v-container>
        ...
        <v-card v-if="this.$store.state.user.logged === false">
            <v-card-title class="mx-2 mt-5"> Necessario ruolo da allevatore per proseguire </v-card-title>
            <v-card-actions>
                <v-text-field v-model="pk"  placeholder="chiave" class="mx-2 mt-5"/>
                <v-btn type="button" class="btn btn-primary mx-2" v-on:click="doLogin"> Login </v-btn>
            </v-card-actions>
        </v-card>
        <v-card v-if="this.$store.state.user.logged === true">
            <v-card-title> Loggato come: {{this.$store.state.user.address}} </v-card-title>
            <v-card-title> CF: {{this.$store.state.user.CF}} </v-card-title>
            <v-card-subtitle> Inserisci i parametri della mucca: </v-card-subtitle>
            <v-card-actions>
                <v-combobox :items="this.$store.state.cowsList" class="mx-2" v-model="cow.id"  placeholder="marca auricolare"/>
                <v-text-field type="text" class="mx-2" v-model="cow.color"  placeholder="colore mucca"/>
                <v-text-field type="number" class="mx-2" v-model="cow.age"  placeholder="età mucca"/>
                <v-text-field type="text" class="mx-2" v-model="cow.razza"  placeholder="razza mucca"/>
